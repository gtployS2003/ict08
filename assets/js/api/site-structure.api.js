// assets/js/api/site-structure.api.js
(() => {
  const API_BASE =
    window.API_BASE_URL ||
    window.__API_BASE__ ||
  ...
  async function list({ q = "", page = 1, limit = 50, isPublic = false } = {}) {
    ...
    return apiFetchCompat(`/site-structures${qs.toString() ? `?${qs.toString()}` : ""}`, {
      method: "GET",
      skipAuth: !!isPublic,
    });
  }
  ...
  async function update(id, payload) {
    ...
    // Use POST + method override so PHP can parse multipart/form-data into $_POST/$_FILES
    // (native PUT/PATCH multipart is not reliably parsed by PHP).
    try {
    ...
    return apiFetchCompat(`/site-structures/${Number(id)}`, {
      method: "POST",
  ...
  async function remove(id) {
    if (id === undefined || id === null || id === "") throw new Error("id is required");
    return apiFetchCompat(`/site-structures/${Number(id)}`, { method: "DELETE" });
  }
